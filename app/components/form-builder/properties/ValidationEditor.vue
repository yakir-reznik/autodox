<script setup lang="ts">
	...
	const showPattern = computed(() => ["text", "email"].includes(props.element.type));
			...
			<div class="grid grid-cols-2 gap-3">
				<div>
					...
					<BaseInput
						...
						placeholder="0"
						@update:model-value="
							updateValidation({ minLength: $event ? Number($event) : undefined })
						"
					/>
				</div>
				<div>
					...
					<BaseInput
						...
						placeholder="∞"
						@update:model-value="
							updateValidation({ maxLength: $event ? Number($event) : undefined })
						"
					/>
				</div>
			</div>
			...
			<div class="grid grid-cols-2 gap-3">
				<div>
					...
					<BaseInput
						...
						placeholder="-∞"
						@update:model-value="
							updateValidation({ min: $event !== '' ? Number($event) : undefined })
						"
					/>
				</div>
				<div>
					...
					<BaseInput
						...
						placeholder="∞"
						@update:model-value="
							updateValidation({ max: $event !== '' ? Number($event) : undefined })
						"
					/>
				</div>
			</div>
