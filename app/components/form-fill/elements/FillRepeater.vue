...
<template>
	<div :class="containerClass">
		...
		<label
			v-if="config.label"
			class="form-fill-label block text-sm font-medium text-foreground mb-2"
		>
			{{ config.label }}
		</label>
			...
			<TransitionGroup name="fade-scale-move">
				...
				>
					...
					<div class="form-fill-repeater-header flex items-center justify-between mb-3">
						<span
							class="form-fill-repeater-index text-sm font-medium text-muted-foreground"
						>
							{{
								config.itemName
									? `${config.itemName} #${itemIndex + 1}`
									: `#${itemIndex + 1}`
							}}
						</span>
						<button
