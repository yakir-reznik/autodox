...
const {
	...
	error,
	refresh,
} = await useFetch<DetailsResponse>(
	...
	<div dir="rtl" class="min-h-screen bg-gray-100">
		...
		<main class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
			...
			<div v-else-if="error" class="rounded-lg bg-red-50 p-6 text-center">
				...
				<p class="mt-2 text-red-700">{{ error.message || "Failed to load submission details" }}</p>
			...
			<div v-else-if="submission">
				...
				<div class="mb-8 rounded-lg bg-white p-6 shadow">
					<div class="grid grid-cols-1 gap-6 md:grid-cols-3">
						...
						<div>
							...
							<div class="mt-2">
								<NuxtLink :to="`/fill/${submission.formId}`" class="text-blue-600 hover:underline">
						...
						<div class="mt-4 space-y-4">
							<div class="flex gap-4">
								...
								<div class="flex-1">
									...
									<p class="text-sm text-gray-600">{{ formatDate(submission.createdAt) }}</p>
								</div>
							</div>
							...
							<div v-if="submission.startedAt" class="flex gap-4">
								<div class="flex flex-col items-center">
									<div class="flex h-8 w-8 items-center justify-center rounded-full bg-blue-100">
										<Icon name="heroicons:arrow-right-circle" class="h-4 w-4 text-blue-600" />
									</div>
								</div>
								<div class="flex-1">
									<p class="text-sm font-medium text-gray-900">Form Started</p>
									<p class="text-sm text-gray-600">{{ formatDate(submission.startedAt) }}</p>
								</div>
							</div>
							...
							<div v-if="submission.lockedAt" class="flex gap-4">
								...
								<div class="flex-1">
									...
									<p class="text-sm text-gray-600">{{ formatDate(submission.lockedAt) }}</p>
								</div>
							</div>
						</div>
					</div>
				...
				<div v-if="submission.submissionData" class="mb-8 rounded-lg bg-white p-6 shadow">
					...
					<div dir="ltr" class="overflow-x-auto rounded-lg bg-gray-50 p-4 font-mono text-sm">
					...
					<div class="mt-4 flex gap-2">
						<UiButton
							variant="secondary"
							size="sm"
							@click="copyToClipboard(JSON.stringify(submission.submissionData, null, 2), 'Submission data')"
						>
				...
				<div v-if="submission.prefillData" class="mb-8 rounded-lg bg-white p-6 shadow">
					...
					<div class="mt-4 flex gap-2">
						<UiButton
							variant="secondary"
							size="sm"
							@click="copyToClipboard(JSON.stringify(submission.prefillData, null, 2), 'Prefill data')"
						>
							<Icon name="heroicons:clipboard-document" class="h-4 w-4" />
							Copy JSON
						</UiButton>
				...
				<div v-if="submission.additionalData" class="mb-8 rounded-lg bg-white p-6 shadow">
					...
					<div class="mt-4 flex gap-2">
						<UiButton
							variant="secondary"
							size="sm"
							@click="copyToClipboard(JSON.stringify(submission.additionalData, null, 2), 'Additional data')"
						>
							<Icon name="heroicons:clipboard-document" class="h-4 w-4" />
							Copy JSON
						</UiButton>
				...
				<div v-if="entrances.length > 0" class="mb-8 rounded-lg bg-white p-6 shadow">
					<h2 class="mb-4 text-lg font-medium text-gray-900">Form Entrances ({{ entrances.length }})</h2>
					<div class="overflow-x-auto">
						<table class="w-full">
							...
							<tbody class="divide-y divide-gray-200">
								<tr v-for="(entrance, index) in entrances" :key="entrance.id" class="hover:bg-gray-50">
									...
									<td class="px-4 py-3 text-center text-sm">
										<code v-if="entrance.ipAddress" class="rounded bg-gray-100 px-2 py-1 font-mono text-xs text-gray-700">
											{{ entrance.ipAddress }}
										</code>
									...
									<td class="px-4 py-3 text-center text-sm">
										...
										/>
										<Icon v-else name="heroicons:x-circle" class="mx-auto h-5 w-5 text-gray-400" />
									</td>
								</tr>
							</tbody>
						</table>
